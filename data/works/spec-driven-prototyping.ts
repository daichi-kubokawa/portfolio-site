...
export const specDrivenPrototyping: Work = {
  ...
  content: [
...

    {
      ...
      title: "振り返り",
      titleFont: "sans",
      body: "本プロジェクトを通して、生成AIを開発に取り入れるうえでは、プロンプトの工夫だけでなく、「どの工程で何を判断させ、どこで人間が確認するか」というプロセス全体の設計が重要だと実感しました。\n\nまた、実際にワークフローを動かして計測・検証したことで、設計時には見えなかったボトルネックや品質上の課題を発見し、改善につなげることができました。AIに任せる範囲を広げるだけではなく、判断根拠を残し、検証可能な状態で次の工程へつなぐことが、継続的に活用できる仕組みには必要だと考えています。\n\n今回はヒアリングから仕様化、プロトタイプによる検証までを対象としました。今後は、確定した仕様や設計判断を実装工程へ接続し、生成されたコードと仕様の整合性まで継続して検証できるワークフローへ拡張していきたいと考えています。",
    },
  ],
};
